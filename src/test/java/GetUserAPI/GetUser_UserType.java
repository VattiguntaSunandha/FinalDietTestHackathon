...
public class GetUser_UserType {
	...
	public void user_is_on_get_request_with(String endpoint) {
		...
		RestAssured.basePath = ApplicationHook.prop.getProperty("patientEndpoint");
		
		request = RestAssured.given().auth().preemptive().basic(ApplicationHook.prop.getProperty("username_valid"), ApplicationHook.prop.getProperty("password_valid")).contentType("application/json"); 
	...
	public void user_is_on_get_user_type_request() {
		...
		request = RestAssured.given().auth().preemptive().basic(ApplicationHook.prop.getProperty("username_invalid"), ApplicationHook.prop.getProperty("password_invalid")).contentType("application/json"); 
	...
	public void status_code_internal_server_error_will_be_displayed(Integer status) {
	    ...
	    if (status==statuscode) {
	    	...
	    	System.out.println("*************Servers are up and running. Sanity testing successful***********");
	}
